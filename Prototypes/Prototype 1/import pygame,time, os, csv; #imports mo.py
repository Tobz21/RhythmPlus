import pygame,time, os, csv; #imports modules listed
...
judgeLine=pygame.Rect(420, 50, 400, 30) #variable for control of judgement line display
...
def draw_text(text, font, text_col, x, y): #function taking text
...
#sound engine/class, takes parameters volumes, sfx state and chart
class Sounds:
...
#methods to play corresponding judgement sounds
    ...
    def missHit(self):
        self._missHit.play()
#method to redirect to judgement sounds depending on the judgement received 
...
class Note: #define the note class
    ...
    def checkInput(self, left, up, down, right, time):
        if ((self._direction=='LEFT') and (left)) or ((self._direction=='DOWN') and (down)) or ((self._direction=='UP') and (up)) or ((self._direction=='RIGHT') and (right)): #test if key pressed and matches note key
...
#class of statistics to store statistic variables and update them
class Statistics:
    ...
    def accuracyDraw(self):
        draw_text((str(self._accuracy) + "%"), scoreFont, (255,255,255) ,1100, 100)
    ...
    def healthDraw(self):
        draw_text(str(self._health), scoreFont, (0,255,0) ,1100, 0)
    ...
    def scoreDraw(self):
        draw_text(str(self._score), scoreFont, (255,255,255) ,1100, 20)
...
while run: #loop that is active when game loop flag is True
    ...
    if start: #if it is the loop's first run
        ...
        unspawnedNotes = []
        for note in notes:
            note[0]= float(note[0]) + 1
            unspawnedNotes.append(note)
    ...
    for event in pygame.event.get(): #loops through events
        if event.type == pygame.QUIT: #if a quit type event occurs
            run = False #set game loop flag to False

    #key input check
    pressed= pygame.key.get_pressed() #get state of all keys (in a list) and store as variable  
    lState=pressed[pygame.K_LEFT] #store left arrow state
    uState=pressed[pygame.K_UP] #store up arrow state
    dState=pressed[pygame.K_DOWN] #store down arrow state
    rState=pressed[pygame.K_RIGHT] #store right arrow state
    ...
    if lState==True and lprevious==False:
        lhit=True
    else:
        lhit=False
    if uState==True and uprevious==False:
        uhit=True
    else:
        uhit=False
    if dState==True and dprevious==False:
        dhit=True
    else:
        dhit=False
    if rState==True and rprevious==False:
        rhit=True
    else:
        rhit=False
    ...
    for noteInstance in spawnedNotes: #for every note instance in the spawnedNotes list
        ...
        noteState=noteInstance.checkInput(lhit,uhit,dhit,rhit, elapsed_time) 
        if noteState: #if note should be acted upon
            ...
            if noteInstance.checkOut(elapsed_time): #if note is too late
                ...
                SoundE.missHit() #play miss sound
                del spawnedNotes[index]  #despawn the note
    ...
    if len(unspawnedNotes)==0 and len(spawnedNotes)==0:
        run=False #changes run state of game loop
