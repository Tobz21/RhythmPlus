import pygame,time,os, os.path, csv,pygame_menu,sqlite3,random; #imports modules listed
...
chartList ={0:["Easy: On and On LVL:3",3], 1:["Medium: Invincible LVL:5",5],2:["Hard: Different Heaven LVL:8",8],3:["Skill tracking chart",10]}
chartIndex=0
...
#hold arrow images
lHoldImg = pygame.image.load('l_arrow_hold.png').convert_alpha()
lHoldImg = pygame.transform.scale(lHoldImg, (65,60))
uHoldImg = pygame.image.load('u_arrow_hold.png').convert_alpha()
uHoldImg = pygame.transform.scale(uHoldImg, (65,60))
dHoldImg = pygame.image.load('d_arrow_hold.png').convert_alpha()
dHoldImg = pygame.transform.scale(dHoldImg, (65,60))
rHoldImg = pygame.image.load('r_arrow_hold.png').convert_alpha()
rHoldImg = pygame.transform.scale(rHoldImg, (65,60))
...
def performanceCalc(notes,score,health):
    global chartList
    global chartIndex
    ...
    if chartIndex!=3:
        ...
        global dead
        global deadTime
        if dead: #if player died on chart
            #time lived is the difference between dead time and chart runtime
            timeLived= totalTime - deadTime
        else: #if player didn't die on chart
            #time lived is the chart runtime
            timeLived = totalTime
        #calculate skill chart performance
        skillRatingChartPerf = (timeLived/totalTime) + 2*(score/maxscore) + (health/1000)
        #convert to actual performance
        performance= (skillRatingChartPerf/4)*10
        maxPerf=10
...
#selector widget
chartMenu.add.selector('Select a chart', items=[(chartList[0][0],0),(chartList[1][0],1),(chartList[2][0],2),(chartList[3][0],3)], onchange=changeChart)
    ...
    if chartIndex!=3:
        ...
        with open(filename, "r", encoding='utf-8-sig') as csvfile: #open the file as reading
            ...
            for row in datareader: #loop through rows in the chart file
                if (len(row)>2 and row[2].strip()=="") or len(row)==2: #if note has 2 elements
                    toAdd=[row[0].strip(),row[1].strip()]
                    returnNotes.append(toAdd) #add tap note
                elif len(row)>=3: #if note has 3 elements
                    toAdd=[] #init array 
                    for i in range(3): #loop to add 3 elements to array
                        toAdd.append(row[i].strip()) #add element
                    returnNotes.append(toAdd) #add hold note
    else:
        global lastDynamicChart
        time=0
        lastDynamicChart=[]
        difference1=2
        while time<30:
            newNote=[]
            noteType=random.randint(1,4)
            if noteType==4:
                time1=str(formatNumber(round2dp(random.uniform(time + 0.14 , time + 0.14 + difference1)),2))
                time2=str(formatNumber(round2dp(random.uniform(float(time1) + 0.14 , float(time) + 0.14 + difference1)),2))
                newNote.append(time1)
                newNote.append(time2)
            else:
                time1=str(formatNumber(round2dp(random.uniform(time + 0.2 , time + 0.2 + difference1)),2))
                newNote.append(time1)
            ...
            if direction==1:
                ...
                direction="RIGHT"
            newNote.append(direction)
            lastDynamicChart.append(newNote)
            if noteType==4:
                time= float(time2)
            else:
                time = float(time1)
            difference1 = difference1 - 0.06
            if difference1<=0.02:
                difference1=0.02
        returnNotes=lastDynamicChart
    global chartLen
    chartLen=0
    for note in returnNotes: #loop through notes
        if len(note)==3: #if note is tap note
            #increments sumN by rounded how many seconds hold note*interval per second
            chartLen = chartLen + int(round(float(note[1])-float(note[0])))*4
        else: #if tap note its just +1
            chartLen = chartLen + 1
    global totalTime
    lastNote=returnNotes[len(returnNotes)-1]
    if len(lastNote)==3:
        totalTime= float(lastNote[1])
    else:
        totalTime=float(lastNote[0]) 
...
#function to get the note length with hold note consideration
def getNoteLength(chartIndex):
    global chartLen
    return chartLen #returns number of notes in the chart
#draw text fonts
comboFont = pygame.font.SysFont("Aharoni", 100) #font setup variable for combo display
scoreFont = pygame.font.SysFont("Candara", 30 , bold = True) #font setup variable for score display
judgeFont = pygame.font.SysFont("Euphemia", 60 , bold = True) #font setup variable for judgement display
timingFont = pygame.font.SysFont("Century Gothic", 30 , bold = True) #font setup variable for timing display
...
class Sounds:
    ...
    def __init__(self, musicVol, sfxVol, sfxState, chart): 
        ...
        self._chartN=chart
        if chart!=3: #if not skill tracking chart
    ...
    def playChart(self):
        if self._chartN!=3: #if not skill tracking chart
...
class Note: 
    def __init__(self, time, direction): #constructor method for note class, takes 2 parameters, note time and direction/conveyor
        if direction=="LEFT": #if left pointing note
            x = 510 #starting x position will be furthest to the left
            self._image = lImg #set image to left arrow
        elif direction=="DOWN": #if down pointing note
            x = 575 #starting x position will be second from the left
            ...
            x = 705 #starting x pos will be furthest away and after up conveyor
...
#hold note class to move, draw and get judgement data of notes
class HoldNote(Note):
    def __init__(self, time1, time2, direction): #constructor method for hold note, parameters: start time, endtime and a direction
        #call original initialization of parent class Note
        super().__init__(time1, direction)
        #initialization of holdNote exclusively
        self._endTime=float(time2) #set endtime using time 2 and typecast to float
        self._currentState=0 #state 0->not hit yet, 1->being held 2->let go or missed
        self.holdTime= round2dp(float(time2) - float (time1))
        self._rect=pygame.Rect(self._x,self._y, 65, 0) #middle component rect
        #directional assignment for hold notes
        if direction=="LEFT": #if left pointing note
            self._imageEnd = lHoldImg #set image to left arrow
            self._color = (195, 98, 195) #pinkish colour
        elif direction=="DOWN": #if down pointing note
            self._imageEnd = dHoldImg #set image to left arrow
            self._color = (216, 48, 48) #red colour
        elif direction=="UP": #if up pointing note
            self._imageEnd = uHoldImg #set image to left arrow
            self._color = (110, 192, 184) #teal colour
        elif direction=="RIGHT": #if right pointing note
            self._imageEnd = rHoldImg #set image to left arrow
            self._color = (193, 118, 135) #brownish pink colour
        self._missTime=0 #initialize miss time
        self._hitTime=0 #initialize hit time
        self._y2=620 #initialize secondary y coordinate
    #method to update drawings on the screen of the hold note
    def update(self,time): #(parameter:current time)
        newHoldLength = self._y2 - self._y #get length of hold note to be displayed
        if newHoldLength>0: #if note is long enough to be displayed on the screen
            #initialize rectange used to display mid part of hold note
            self._rect=pygame.Rect(self._x,self._y + 30, 65, newHoldLength)
            pygame.draw.rect(screen,self._color,self._rect) #draw mid part of hold note
            if time>=self._endTime-0.94: #if the end of the hold note is on screen
                screen.blit(self._imageEnd, (self._x, self._y2)) #draw arrow end img
        screen.blit(self._image, (self._x, self._y)) #draw start arrow img regardless
    #method to move the notes when the frame time changes
    def move(self,time): #(parameter:current time)
        if self._currentState!=1: #if not being held
            self._y = self._y + (-10) #top part of note moves up
        if self._endTime-0.94<=time: #if endpart on screen
            self._y2 = self._y2 + (-10) #end part moves up
    #method to check if note has completely passed its time
    def checkOut(self, time):#(parameter:current time)
        if time>self._endTime: #if time passed is beyond hold note time
            return True #yes, note is out
    #method to check if note should be assigned miss judgement
    def assignMiss(self, time): #(parameter:current time)
        if self._currentState==0: #if hasn't been hit 
            if time> self._time + 0.15: #if first hit hasnt pressed in time
                self._currentState=2 #note is being missed
                self._missTime=time #new miss time
                print("detected miss through state override")
                return True #can get miss judgement
        elif self._currentState==2: #if note is being missed
            if time> self._missTime + 0.25: #if time went by by 0.25 second of missing
                self._missTime=time #new miss time
                print("detected miss through no change to inputs")
                return True #can get miss judgement
                
    #method to return state for conditions in main code
    def getState(self):
        return self._currentState

    #method to check if note is being held on the frame and should recieve a perfect judgement
    def checkHeld(self,time,holdHits): #(parameters:current time, hits array with no input sanitization)
        if self._currentState!=0: #if note should check for inputs
            #check if any direction of note has its corresponding state of key held, if so then continue
            if ((self._direction=='LEFT') and (holdHits[0])) or ((self._direction=='DOWN') and (holdHits[2])) or ((self._direction=='UP') and (holdHits[1])) or ((self._direction=='RIGHT') and (holdHits[3])):
                if self._currentState==1: #if the note is being held
                    if time > self._hitTime + 0.25: #if note been held for 0.25 seconds
                        self._hitTime=time #reset hit time to current time
                        return True #gets additional perfect judgement
                elif self._currentState==2: #if note is being missed
                    if time > self._hitTime + 0.25: #if note been held for 0.25 seconds
                        self._hitTime=time #reset hit time
                        self._currentState=1 #note is being held
                        self._y = 50 #set note top position to judge line
                        return True #gets additional perfect judgement
            else: #if no input
                self._currentState=2 #set state to missed

    #method to set held state from state 0->1 on first check
    def setHeld(self,time): #(parameter:current time)
        print("hold note is now hold state")
        self._currentState=1 #set state to held
        self._y= 50 #set top position to judgeline
        self._hitTime=time #reset hit time
...
class Statistics:
    def __init__(self, notes,accDisp,death,progDisp,wordJudge,laEaDisp): #initialize class and its variables
        ...
        if len(notes[len(notes)-1])==3:
            self._endTime = float(notes[len(notes)-1][1]) + 0.15
        else:
            self._endTime = float(notes[len(notes)-1][0]) + 0.15
        self._progress=0
        self._progressOutline=pygame.Rect(440, 30, 400, 20)
    ...
    def judgeHandle(self,judgement):
        ...
        # sets judgement colour for drawing
        if judgement=="PERFECT":
            self.perfect() 
            number=300
            self._judgeColour=(255, 210, 0)
        if judgement=="GREAT":
            self.great()
            number=150
            self._judgeColour=(2, 212, 34)
        if judgement=="GOOD":
            self.good()
            number=100
            self._judgeColour=(6, 153, 209)
        if judgement=="BAD":
            self.bad()
            number=50
            self._judgeColour=(189, 25, 137)
    ...
    def accuracyDraw(self):
        draw_text(("Accuracy: " + str(self._accuracy) + "%"), scoreFont, (112, 0, 68) ,1000, 450)
    ...
    def progressDraw(self):
        ...
        progressMeter=pygame.Rect(440, 30, size, 20) #uses size as length, defines as rectangle
        pygame.draw.rect(screen,(19, 7, 242),progressMeter,) #draws rectangle using progress meter and blue colour
        pygame.draw.rect(screen,(255,255,255),self._progressOutline, width=2) #draw progress meter outline, white 
        draw_text("Progress:",scoreFont,(19, 7, 242),580,1)  #draw progress label
    ...
    def healthDraw(self):
        draw_text(("Health: " + str(self._health)), scoreFont, (0,255,0) ,1000, 150)
    ...
    def judgeDraw(self,time):
        ...
        if self._judgeName!="": #if there is a judgement type given
            ...
            elif self._judgeStart + 0.5 > time: #if 0.5 seconds hasnt passed since old judgement
                draw = True #can draw the judgement
        if draw: #if can draw | draw is true
            draw_text(self._judgeName, judgeFont, self._judgeColour ,540, 520)
    
    #method to update combo drawings
    def comboDraw(self):
        if self._currentCombo >= 2: #if current combo is greater than 2
            #draws combo text
            draw_text("COMBO", scoreFont, (145, 102, 255) ,550, 410)
            #draws combo value
            draw_text(str(self._currentCombo), comboFont, (145, 102, 255) ,550, 350)
    
    #method to draw the score onto the screen
    def scoreDraw(self):
        draw_text(("Score: " + str(self._score)), scoreFont, (39, 235, 255) ,1000, 300)
...
while run: #loop that is active when game loop flag is True
    ...
    while gameState:    
        ...
        for note in unspawnedNotes: #for each note array inside the unspawnedNotes list
            if (float(note[0])-0.94)<=elapsed_time: #if it is time to spawn
                #add note to list of spawned notes
                if len(note)==2: #if tap note
                    #create tap note obj
                    spawnedNotes.append(Note(note[0], note[1]))
                elif len(note)==3: #if hold note
                    #create hold note obj
                    spawnedNotes.append(HoldNote(note[0], note[1],note[2]))
                unspawnedNotes.remove(note) #remove note from list of unspawned notes
        #rendering
        screen.fill((0,0,0)) #fill screen with color black
        screen.blit(jLineImg, (510, 50))
        #draw chart name to screen
        draw_text("Chart: "+ chartList[chartIndex][0],scoreFont,(255,255,255),10,50)
        #draw fps
        draw_text("FPS:" + str(round(clock.get_fps())) ,scoreFont,(252, 115, 3),20,10)
        #draw note section lines
        pygame.draw.line(screen,(255,255,255),(430,0),(430,720),width=5)
        pygame.draw.line(screen,(255,255,255),(850,0),(850,720),width=5)
        ...
        for noteInstance in spawnedNotes: #for every note instance in the spawnedNotes list
             #note should move 
            if isinstance(noteInstance,HoldNote):
                noteInstance.update(elapsed_time)
                noteInstance.move(elapsed_time) #call update method for each instance
                if noteInstance.getState()==0: #if note state is initial
                    print("hold note does its first check")
                    firstCheck=True #do checkinput
                else:
                    firstCheck-False #dont do checkinput
            else:
                noteInstance.update()
                noteInstance.move() 
                firstCheck=True #do checkinput
            ...
            if firstCheck: #if do checkinput
                noteState=noteInstance.checkInput(hits, elapsed_time) 
            else:
                noteState=False #dont do note input judgement actions
            if noteState: #if note should be acted upon 
                #for hold notes this only can be done before their first judgement
                ...
                if isinstance(noteInstance,HoldNote):
                    noteInstance.setHeld(elapsed_time)
                    print("hold detected as held")
                else:
                    del spawnedNotes[index]  #despawn the note
                    print("note exterminated(tap)")
            elif isinstance(noteInstance,HoldNote): #if note is a hold note
                if noteInstance.checkOut(elapsed_time):
                    del spawnedNotes[index]
                    print("note exterminated (hold)")
                elif noteInstance.checkHeld(elapsed_time,previous):
                    SoundE.sfxHandle(playStats.judgeHandle("PERFECT"))
                    print("note perfect (hold)")
                elif noteInstance.assignMiss(elapsed_time):
                    playStats.miss() #record miss
                    SoundE.missHit() #play miss sfx
                    print("note missed (hold)")
            elif noteInstance.checkOut(elapsed_time): #if tap note is too late
                playStats.miss() #record miss
                SoundE.missHit() #play miss sfx
                del spawnedNotes[index]  #despawn the note
                print("Note deleted (tap)")
        ...
        if len(unspawnedNotes)==0 and len(spawnedNotes)==0:
            ...
            screen.fill((0,0,0))
            #draw loading screen
            draw_text("Chart Complete!", comboFont, (255,255,255) ,400, 330)
            draw_text("Loading reports screen...", comboFont, (255,255,255) ,260, 420)
            dead=False
        #stopping condition 2- if player died/health<=0
            ...
            screen.fill((0,0,0))
            #draw loading screen
            draw_text("Chart Failed!", comboFont, (255,255,255) ,400, 330)
            draw_text("Loading reports screen...", comboFont, (255,255,255) ,260, 420)
            dead=True
    ...
    if report:
        ...
        #calculations
        noOfNotes=getNoteLength(chartIndex)
        print(noOfNotes)                   
        ...
        performance=performanceCalc(noOfNotes,stats[7][1],stats[10][1]) #calculate performance tuple
        if saveType==0: #if player is guest
            ...
            newPack = reportMenu.add.label(line,padding=0)
            newPack.set_max_width(300)
            frameContentR.pack(newPack,margin=(2,2),align=pygame_menu.locals.ALIGN_CENTER)
